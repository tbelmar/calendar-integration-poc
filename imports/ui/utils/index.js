...
const AWS_EC2_IP = "44.213.248.25"; // This value is subject to change
...
export const createEventLinks = (startTimeStamp, endTimeStamp, location, description, summary) => {
  ...
  // create google link
  const googleLink =
    `https://calendar.google.com/calendar/render?${qs.stringify(googleObj)}`;
  ...
  // create outlook link
  const outlookLink =
    `https://outlook.live.com/calendar/0/deeplink/compose?${qs.stringify(outlookObj)}`;
  ...
  const icalLink = `http://${AWS_EC2_IP}:3000/calendar?${qs.stringify(icalObj)}`;
